...
class PoseContoller(TurtleBot):
    ...
    def move2Goal(self):
        ...
        if self.goal_reached == False:
            ...
            angle_diff = (theta- self.goalPose.theta)
            ...
            if np.abs(angle_diff) > pose_tolerance:
             ...
             # Stopping our robot after the movement is over.
                ...
                vel_msg.angular.z = self.angular_vel(constant = 0.5, th = - angle_diff )

                self.velocity_publisher.publish(vel_msg)

                #print(vel_msg.angular.z)
                #print(self.goalPose.theta, theta, angle_diff )
...
def main(args=None):
    ...
    turtlebot_controller = PoseContoller()

    if len(sys.argv) < 4:
        print("Usage: ros2 run <package_name> <node_name> <x_goal> <y_goal> <tolerance>")
        return
    ...
    if (theta_goal < 0):
        while (theta_goal < 0):
            theta_goal = theta_goal + 2.0*np.pi

    theta_goal = theta_goal - (2.0 * np.pi) * int(theta_goal/ (2.0 * np.pi)) - np.pi
